...
def api_save_config():
    ...
    try:
        ...
        # Create .env file
        success = ConfigManager.create_env_file(config)
...
def main():
    ...
    print(f"Admin/Sudo: {'Yes ✓' if checks['is_admin'] else 'No ✗'}")
    print()
    ...
    if not checks['is_admin']:
        warnings.append(
            "⚠️  Not running with admin/sudo privileges - Docker and Tailscale commands may fail")
    if not docker['installed']:
        warnings.append(
            "⚠️  Docker is not installed - Install from https://docker.com")
    elif not docker['running']:
        warnings.append(
            "⚠️  Docker is not running - Please start Docker Desktop")
    if not compose['installed']:
        warnings.append("⚠️  Docker Compose is not installed")
    if not tailscale['installed']:
        warnings.append(
            "⚠️  Tailscale is not installed - Install from https://tailscale.com")
    elif not tailscale['running']:
        warnings.append(
            "⚠️  Tailscale is not running - Please start Tailscale")
    ...
    print(f"   \033[1;36mhttp://localhost:8080\033[0m\n")
    print("=" * 63)
